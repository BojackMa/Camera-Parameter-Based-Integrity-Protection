...
# CUDA_VISIBLE_DEVICES=4,5 nohup python -u handledata_TMC_koulian_4wei.py > log1126_4wei_test.txt 2>&1 &
import cv2
import os
import numpy as np
import random
...
def process_pair(i, timestamps_exposure, frame_list):
    ...
    for j in range(4):
        deltat = 0
        for k in range(8):
            if (prev_row[j*8+k+1]!=curr_row[j*8+k+1]):
                # delta[(j-1)//8] = quantize_delta(curr_row[j] - prev_row[j])
                deltat +=  quantize_delta(curr_row[j*8+k+1] - prev_row[j*8+k+1])
        delta[j] = quantize_delta(deltat,0.1) * 10 
        if (delta[j] != 0 ):
            flag = 1
    ...
    #             # delta[(j-1)//8] = quantize_delta(curr_row[j] - prev_row[j])
    #             deltat +=  quantize_delta(curr_row[j*8+k+1] - prev_row[j*8+k+1])
    #     delta[j] = quantize_delta(deltat,0.4) * 10 
    #     if (delta[j] != 0 ):
    #         flag = 1
...
# dcim_dir = os.path.join(base_dir, 'DCIM_1126')
# txt_dir = os.path.join(base_dir, 'files_1126')

# scan_file_and_handle(dcim_dir,txt_dir)

func("VID_1764094675397.mp4","1764094675397.txt")
